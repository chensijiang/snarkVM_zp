...
fn main() {
    ...
    if nvcc.is_ok() {
        ...
        nvcc.flag("-Xcompiler").flag("-Wno-unused-parameter"); // ntt
        nvcc.flag("-Xcompiler").flag("-Wno-subobject-linkage"); // ntt
        ...
        nvcc.file("cuda/snarkvm.cu").compile("snarkvm_cuda");

        println!("cargo:rustc-cfg=feature=\"cuda\"");
        println!("cargo:rerun-if-changed=cuda");
        println!("cargo:rerun-if-changed=ntt-cuda");
        println!("cargo:rerun-if-changed=poly-cuda");
